-- Reporting & dashboard schema update
-- Adds the customer fields used by merchant customer insights
-- and the merchant business fields used by admin merchant reporting.

ALTER TABLE users
    ADD COLUMN postal_code VARCHAR(6) DEFAULT NULL AFTER gender,
    ADD COLUMN preferred_contact_method ENUM('email','phone','whatsapp') DEFAULT NULL AFTER postal_code;

ALTER TABLE salons
    ADD COLUMN business_category VARCHAR(80) DEFAULT NULL AFTER salon_name,
    ADD COLUMN uen VARCHAR(20) DEFAULT NULL AFTER business_category,
    ADD COLUMN years_in_business INT DEFAULT NULL AFTER uen,
    ADD COLUMN staff_count INT DEFAULT NULL AFTER years_in_business;

-- Optional starter data for the sample merchants already present in the dump.
-- Change these values if your team's actual merchant details are different.
UPDATE users
SET phone = CASE email
    WHEN 'beauty@vaniday.sg' THEN '81234561'
    WHEN 'spa@vaniday.sg' THEN '81234562'
    WHEN 'barber@vaniday.sg' THEN '81234563'
    ELSE phone
END
WHERE email IN ('beauty@vaniday.sg', 'spa@vaniday.sg', 'barber@vaniday.sg')
  AND (phone IS NULL OR phone = '');

UPDATE salons
SET
    business_category = CASE salon_id
        WHEN 1 THEN 'Beauty Salon'
        WHEN 2 THEN 'Spa'
        WHEN 3 THEN 'Barber'
        ELSE business_category
    END,
    uen = CASE salon_id
        WHEN 1 THEN '202600001A'
        WHEN 2 THEN '202600002B'
        WHEN 3 THEN '202600003C'
        ELSE uen
    END,
    years_in_business = CASE salon_id
        WHEN 1 THEN 6
        WHEN 2 THEN 4
        WHEN 3 THEN 5
        ELSE years_in_business
    END,
    staff_count = CASE salon_id
        WHEN 1 THEN 8
        WHEN 2 THEN 6
        WHEN 3 THEN 4
        ELSE staff_count
    END
WHERE salon_id IN (1, 2, 3);
...
INSERT INTO `salons` VALUES (1,3,'Vaniday Beauty Studio','Orchard','Hair styling, facials, and beauty treatments.',NULL,15.00,NULL),(2,4,'FreshGlow Spa','Tampines','Relaxing spa and body treatments.',NULL,15.00,NULL),(3,5,'Urban Groom Barbers','Woodlands','Haircuts, fades, and grooming services.',NULL,15.00,NULL),(4,2,'Vaniday Beauty Studio','Singapore','Merchant storefront for product listings.',NULL,15.00,NULL);
